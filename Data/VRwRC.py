...
class CreateDemandCallback(object):
  ...
  def Demand(self, from_node, to_node):
    return self.matrix[from_node][0]
...
class CreateServiceTimeCallback(object):
  ...
  def ServiceTime(self, from_node, to_node):
    return self.matrix[from_node][0] * self.time_per_demand_unit

# Create total_time callback (equals service time plus travel time).
class CreateTotalTimeCallback(object):
  def __init__(self, service_time_callback, travel_time):
    self.service_time_callback = service_time_callback
    self.travel_time = travel_time

  def TotalTime(self, from_node, to_node):
    return self.service_time_callback(from_node, to_node) + self.travel_time(from_node, to_node)
...
def main():
  ...
  locations = dataIMC.L
  demands = dataIMC.demands
  start_times = dataIMC.Time
  travel_time = dataIMC.T
  num_locations = len(locations)
  depot = 0
  num_vehicles = dataIMC.V
  search_time_limit = 4000
  ...
  if num_locations > 0:
    ...
    total_times = CreateTotalTimeCallback(service_time_callback, travel_time)
